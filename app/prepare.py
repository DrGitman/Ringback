...
def _system_instruction(query: str, student: Optional[Student], tenant: dict, office_keys: List[str]) -> str:
    ...
    return (
        ...
        "compound question.\n\n"
        "Set should_call to false only when the request itself cannot be resolved over "
        "the phone at all: it needs someone to see the account directly, needs in-person "
        "identity verification, or is one of - a caller who is not the student themselves, "
        "a payment arrangement, appeal, deferral, cancellation, name/ID correction, "
        "student number recovery, transcript request, disability accommodation, contact "
        "detail change, a complaint, a disciplinary/legal/medical matter, or genuine "
        "distress needing a person. Pick the right office from: "
        f"{', '.join(office_keys)}. A question the knowledge base simply doesn't cover is "
        "NOT one of these - keep should_call true; CALL-E will listen, say honestly that "
        "nothing covers it, and report the case unresolved so the right office follows up "
        "afterward. Otherwise write the briefing paragraph yourself, grounded only in what "
